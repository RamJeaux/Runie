...
- Runie Tokens are earned exclusively through your own normal OSRS play. There is no way to buy them, trade them, or transfer them.
- A Summon is a free cosmetic unlock mechanic: nothing of real-world value changes hands, **every chance-to-find is published transparently in the plugin**, a visible pity system guarantees outcomes, and duplicates always refund Runie Tokens so no Summon is ever a total loss.
...
All Runie data (your collection, currencies, levels, settings) is stored **locally on your computer as JSON**. The plugin makes **zero network requests** and collects **no telemetry, analytics, or personal data**. Nothing ever leaves your machine.
